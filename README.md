...
The picture is generated by [`docs/make_diagram.py`](docs/make_diagram.py)
from the step banners the engines print, so it cannot quietly fall out of
step with the code. `python3 docs/make_diagram.py --check` re-reads
`comprehensive_variant_calling.py` and `fusion_calling.py` and fails if the
step lists no longer match.
