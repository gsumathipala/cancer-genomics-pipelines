...
[![Every step of both pipelines, with PCGR decomposed](docs/pipeline_overview.png)](docs/pipeline_overview.svg)

*Click for the scalable version.* Every step both engines actually run, in
the order they run it, with the tool that does the work named underneath —
and **PCGR broken out into its ten internal stages**, which is the part
normally drawn as a single box labelled "clinical report".

Read it in three columns:

- **Left — the DNA arm.** Fourteen steps, `comprehensive_variant_calling.py`.
  Note that steps 6 and 7 exist only to feed step 9: the strand-bias model
  and the contamination estimate are *inputs to filtering*, not results in
  their own right.
- **Middle — the RNA arm.** Six steps, `fusion_calling.py`. Step 5 is not
  decoration. It answers the question a fusion table cannot: **was this
  library good enough for "no fusion found" to mean anything?**
- **Right — PCGR, decomposed.** Both arms converge on *stage 2*, input
  validation, because PCGR 2.3.2 takes a fusion TSV as a first-class input
  alongside a VCF. One report covers whichever arms were run. The
  oncogenicity bands below it are the ClinGen/CGC/VICC point thresholds
  PCGR actually applies.

The orange boxes down the middle are the **silent-failure guards**, and they
are the reason the pipeline is shaped this way. Each one catches a failure
that would otherwise produce a clean, well-formed, entirely plausible report
that is *wrong* — the empty fusion table that looks exactly like a true
negative, the panel scored against an exome-sized TMB denominator, the
negative result covering regions that were never sequenced deeply enough to
speak for. A crash is cheap by comparison: somebody sees a crash.
