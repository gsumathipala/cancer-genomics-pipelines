...
| **10a** | [Testing a pipeline you cannot run in a test](#10a-testing-a-pipeline-you-cannot-run-in-a-test) |
...
The whole thing, drawn to scale, with PCGR opened up:

[![Every step of both pipelines, with PCGR decomposed](docs/pipeline_overview.png)](docs/pipeline_overview.svg)

That picture is generated by `docs/make_diagram.py` from the step banners
the engines print, and `--check` fails if the two ever disagree. This
matters more than it sounds: a diagram is the first thing a newcomer
trusts and the last thing anyone updates, so one that cannot be verified
is a liability rather than a help. If you take one idea from this document
into your own project, it could reasonably be this one.

The ASCII version below says the same thing in less detail, and is here
because it survives in a terminal, in a diff, and in a commit message.
...
### 10a. Testing a pipeline you cannot run in a test

The obvious objection to testing a bioinformatics pipeline is that a real
run takes hours and 30 GB of reference data, so there is nothing to test.
That objection quietly assumes the tools are the software. They are not:
BWA, STAR, GATK, Arriba and PCGR are third-party programs with their own
test suites, and re-testing them here would buy nothing.

What *is* ours is every decision between those tools — which settings a
...
Three habits from it are worth stealing:

1. **Name the bug in the test.** Every case in `tests/test_regressions.py`
   says which failure it prevents and, where the failure was silent, what
   the user would have seen instead of an error. A test whose purpose is
   not written down becomes something nobody dares delete and nobody
   understands.
2. **A test that fails on arrival is a finding.** Do not adjust either
   side until you know which one is wrong. Two real defects in this
   codebase were found exactly that way — two unlabelled definitions of
   "supporting reads", and a STAR index that discovery accepted on one
   sentinel file while the alignment step demanded four.
3. **Test the properties documentation drifts on.** That every script
   compiles, that the dependency promise still holds, that the README
   names files which exist, that the diagram still matches the code. These
   never fail while you are looking, and always fail eventually.
